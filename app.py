...
def get_quarterly_financials(ticker_symbol):
    try:
        ...
        if quarterly_income.empty:
            return None, None, "No quarterly data available"
        ...
        if not quarterly_earnings.empty and 'EPS' in quarterly_earnings.columns:
            ...
            quarterly_data['EPS'] = [pat / (shares / 1e6) for pat in quarterly_data['PAT']]
        
        # Calculate EPS Growth %
        eps_growth = [0]
        for i in range(1, len(quarterly_data['EPS'])):
            if quarterly_data['EPS'][i-1] != 0:
                growth = ((quarterly_data['EPS'][i] - quarterly_data['EPS'][i-1]) / 
                         abs(quarterly_data['EPS'][i-1])) * 100
            else:
                growth = 0
            eps_growth.append(growth)
        quarterly_data['EPS Growth %'] = eps_growth
    ...
    if not ticker_symbol:
        ...
        if st.button("Analyze Financials"):
            ...
            if financial_df is not None and annual_error is None:
                ...
                if quarterly_df is not None and quarterly_error is None:
                    st.success(f"Quarterly data retrieved for {stock_query} ({ticker_symbol})")
                    
                    # Quarterly Revenue Section
                    st.subheader("1. Quarterly Revenue, Operating Profit & OPM%")
                    
                    q_col1, q_col2 = st.columns([1, 2])
                    
                    with q_col1:
                        st.markdown("**Financial Metrics (Quarterly)**")
                        # Create formatted display table
                        q_display_df = quarterly_df.copy()
                        q_display_df['Revenue'] = q_display_df['Revenue'].apply(format_number)
                        q_display_df['Operating Profit'] = q_display_df['Operating Profit'].apply(format_number)
                        q_display_df['OPM %'] = q_display_df['OPM %'].apply(lambda x: f"{x:.1f}%")
                        
                        st.dataframe(q_display_df[['Revenue', 'Operating Profit', 'OPM %']])
                        
                        # Download button
                        q_csv = quarterly_df[['Revenue', 'Operating Profit', 'OPM %']].to_csv()
                        st.download_button(
                            label="Download Quarterly Revenue Data",
                            data=q_csv,
                            file_name=f"{stock_query}_quarterly_revenue.csv",
                            mime="text/csv"
                        )
                    ...
                    with q_col2:
                        ...
                        ax1.bar(x - width/2, quarterly_df['Revenue']/1e7, width, 
                                label='Revenue (₹ Cr)', color='skyblue')
                        ax1.bar(x + width/2, quarterly_df['Operating Profit']/1e7, width, 
                                label='Op Profit (₹ Cr)', color='lightgreen')
                        ax1.set_ylabel('Amount (₹ Crores)')
                        
                        # Line plot for OPM%
                        ax2 = ax1.twinx()
                        ax2.plot(x, quarterly_df['OPM %'], 'r-o', linewidth=2, label='OPM %')
                        ax2.set_ylabel('OPM %', color='red')
                        ax2.tick_params(axis='y', labelcolor='red')
                        
                        # Set y-axis limits dynamically
                        if max(quarterly_df['OPM %']) > 0:
                            ax2.set_ylim(0, max(quarterly_df['OPM %']) * 1.5)
                        
                        plt.title(f'{stock_query} Quarterly Revenue & Profitability')
                        plt.xticks(x, quarters, rotation=45)
                        ax1.legend(loc='upper left')
                        ax2.legend(loc='upper right')
                        st.pyplot(fig)
                    
                    # Quarterly Analysis insights
                    st.subheader("Quarterly Key Observations")
                    st.markdown(generate_quarterly_analysis(quarterly_df, quarters))
                    
                    # Quarterly Profitability Section
                    st.subheader("2. Quarterly PBT, PAT & EPS Analysis")
                    
                    q_col3, q_col4 = st.columns([1, 2])
                    
                    with q_col3:
                        st.markdown("**Profitability Metrics (Quarterly)**")
                        # Create formatted display table
                        q_display_df2 = quarterly_df.copy()
                        q_display_df2['PBT'] = q_display_df2['PBT'].apply(format_number)
                        q_display_df2['PAT'] = q_display_df2['PAT'].apply(format_number)
                        q_display_df2['EPS'] = q_display_df2['EPS'].apply(lambda x: f"₹{x:.1f}")
                        q_display_df2['EPS Growth %'] = q_display_df2['EPS Growth %'].apply(lambda x: f"{x:.1f}%")
                        
                        st.dataframe(q_display_df2[['PBT', 'PAT', 'EPS', 'EPS Growth %']])
                        
                        # Download button
                        q_csv2 = quarterly_df[['PBT', 'PAT', 'EPS', 'EPS Growth %']].to_csv()
                        st.download_button(
                            label="Download Quarterly Profitability Data",
                            data=q_csv2,
                            file_name=f"{stock_query}_quarterly_profitability.csv",
                            mime="text/csv"
                        )
                    
                    with q_col4:
                        st.markdown("**Quarterly Profitability Trend**")
                        
                        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))
                        
                        # PAT and PBT trend
                        ax1.bar(quarters, quarterly_df['PAT']/1e7, color='lightblue', label='PAT (₹ Cr)')
                        ax1.bar(quarters, (quarterly_df['PBT'] - quarterly_df['PAT'])/1e7, 
                                bottom=quarterly_df['PAT']/1e7, color='lightcoral', label='Tax (₹ Cr)')
                        ax1.set_ylabel('Amount (₹ Crores)')
                        ax1.set_title('Quarterly Profit Composition')
                        ax1.legend()
                        plt.xticks(rotation=45)
                        
                        # EPS Growth
                        ax2.plot(quarters, quarterly_df['EPS'], 'g-o', label='EPS (₹)')
                        ax2.set_ylabel('EPS', color='green')
                        ax2.tick_params(axis='y', labelcolor='green')
                        
                        ax3 = ax2.twinx()
                        ax3.bar(quarters, quarterly_df['EPS Growth %'], alpha=0.3, color='purple', label='EPS Growth %')
                        ax3.set_ylabel('Growth %', color='purple')
                        ax3.tick_params(axis='y', labelcolor='purple')
                        ax3.axhline(0, color='grey', linestyle='--')
                        ax3.set_title('Quarterly EPS Performance')
                        ax3.legend(loc='lower right')
                        plt.xticks(rotation=45)
                    ...
                    st.error(f"Could not retrieve quarterly data: {quarterly_error}")
                ...
                st.error(f"Could not retrieve annual data: {annual_error}")
...
- **Financial Metrics**: 
  - Annual: Revenue, Operating Profit, OPM%, PBT, PAT, EPS
  - Quarterly: Revenue, Operating Profit, OPM%, PBT, PAT, EPS
- **Analysis**: 
  - Annual: 4-year trend with visualizations and key insights
  - Quarterly: Last 6 quarters with visualizations and key insights
- **Number Formatting**: 
  - ₹1,000 = ₹1K 
  - ₹100,000 = ₹1L 
  - ₹10,000,000 = ₹1Cr
